...
def showDriveFiles(users):
  ...
  for user in users:
    ...
    feed = callGAPIpages(service=drive.files(), function=u'list', items=u'items', page_message=page_message, soft_errors=True, q=query, maxResults=1000, fields=fields)
    for file in feed:
      a_file = {u'Owner': user}
...
def doPrintLicenses(return_list=False):
  ...
  for productId in products:
    ...
    try:
      licenses = callGAPIpages(service=lic.licenseAssignments(), function=u'listForProduct', items=u'items', throw_reasons=[u'invalid', u'forbidden'], page_message=page_message, customerId=domain, productId=productId, maxResults=1000)
    except apiclient.errors.HttpError:
      licenses = []
    for license in licenses:
      a_license = dict()
