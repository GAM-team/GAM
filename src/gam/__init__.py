...
def check_num_parents(user, body):
    num_parents = len(body.get('parents', []))
    if num_parents > 1:
        sys.stderr.write(f'Multiple parents ({num_parents}) specified for {user}, only one is allowed.\n')
        return False
    return True
...
def doUpdateDriveFile(users):
    ...
    for user in users:
        ...
        if not check_num_parents(user, body):
            continue
