...
MCP_CACHE_WRITE_FAILED = 'MCP: could not write cache file {0}: {1}\n'
MCP_COMMAND_NEVER_ALLOWED = 'Action {0} writes local files and is not available through MCP'
MCP_COMMAND_NOT_RECOGNIZED = 'Command not recognized: {0}'
MCP_COMMAND_REFUSED = 'Refused: {0}\n{1}'
MCP_COMMAND_REQUIRES_ALLOWWRITES = 'Action {0} is not read-only; the MCP server must be started with allowwrites to run it'
MCP_COMMAND_TIMED_OUT = 'Command did not complete within {0} seconds; GAM commands cannot be interrupted, it continues in the background and gam_run is refused until it finishes'
MCP_DOCUMENT_NOT_AVAILABLE = '{0} is not available locally and could not be fetched: {1}'
MCP_INVALID_PAGE_NAME = 'Invalid wiki page name: {0}'
MCP_NO_COMMAND = 'No GAM command given'
MCP_REASON_AUDIT = 'audit commands manage the email audit monitor and are not available through MCP'
MCP_REASON_BATCH = 'batch, csv, loop and tbatch read local files and run many commands; not available through MCP'
MCP_REASON_CREDENTIALS = 'commands that create, change or select projects, service accounts or their keys write local credential files; not available through MCP'
MCP_REASON_FILE_SELECTOR = 'file based entity selectors read local files; not available through MCP'
MCP_REASON_META = 'redirect, config and multiprocessexit read or write local files or change gam.cfg; not available through MCP'
MCP_REASON_OAUTH = 'oauth commands manage credentials; not available through MCP'
MCP_REASON_SELECT_SAVE = 'select with save (or with no command) writes gam.cfg; not available through MCP'
MCP_REASON_SENDEMAIL = 'sendemail and sendreply send mail; not available through MCP'
MCP_REASON_TODRIVE = 'todrive uploads results to Google Drive; the MCP server must be started with allowwrites to use it'
MCP_SERVER_BUSY = 'A previous gam_run command is still executing; GAM commands cannot be interrupted, retry later'
MCP_SERVER_STARTED = 'GAM {0} MCP server started ({1}); one JSON-RPC message per line on stdin/stdout\n'
MCP_SERVER_STOPPED = 'GAM MCP server stopped\n'
MCP_SYNTAX_LOADED = 'MCP: indexed {0} syntax blocks from {1}\n'
MCP_TEXT_TRUNCATED = '\n[Output truncated: {0} of {1} characters shown]\n'
MCP_TOKEN_NOT_ALLOWED = 'Argument "{0}" (position {1}) is not allowed: {2}'
MCP_UNKNOWN_NOTIFICATION = 'MCP: ignoring notification {0}\n'
MCP_WIKI_FETCH_DISABLED = 'Wiki fetches are disabled (nowiki); only pages in a local checkout or cache are available'
...
ONLY_ADMINISTRATORS_CAN_DISPLAY_SHARED_DRIVE_ORGUNITS = 'Only administrators can display Shared Drive Org Units'
