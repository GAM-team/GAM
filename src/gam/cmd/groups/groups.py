...
def getGroupAttrValue(argument, gs_body):
  if argument == 'copyfrom':
    gs_body[argument] = _getMain().getEmailAddress()
    return
  ...
  if attrType == GC.TYPE_BOOLEAN:
    gs_body[attrName] = str(_getMain().getBoolean()).lower()
  elif attrType == GC.TYPE_STRING:
    if attrName in _getMain().GROUP_FIELDS_WITH_CRS_NLS:
      gs_body[attrName] = _getMain().getStringWithCRsNLs()
    else:
      gs_body[attrName] = _getMain().getString(Cmd.OB_STRING, minLen=0)
  elif attrType == GC.TYPE_CHOICE:
    gs_body[attrName] = _getMain().getChoice(attribute['choices'], mapChoice=True)
  elif attrType in [GC.TYPE_EMAIL, GC.TYPE_EMAIL_OPTIONAL]:
    gs_body[attrName] = _getMain().getEmailAddress(noUid=True, optional=attrType == GC.TYPE_EMAIL_OPTIONAL)
    if attrType == GC.TYPE_EMAIL_OPTIONAL and gs_body[attrName] is None:
      gs_body[attrName] = ''
  elif attrType == GC.TYPE_LANGUAGE:
    gs_body[attrName] = _getMain().getLanguageCode(_getMain().LANGUAGE_CODES_MAP)
  else: # GC.TYPE_INTEGER
    minVal, maxVal = attribute[GC.VAR_LIMITS]
    if attrName == 'maxMessageBytes':
      gs_body[attrName] = _getMain().getMaxMessageBytes(minVal, maxVal)
    else:
      gs_body[attrName] = _getMain().getInteger(minVal, maxVal)
    ...
    if copySettingsFromGroup:
      try:
        if copySettingsFromGroup.find('@') == -1: # group settings API won't take uid so we make sure cd API is used so that we can grab real email.
          copySettingsFromGroup = _getMain().callGAPI(cd.groups(), 'get',
        ...
        if settings is not None:
          ...
          return settings
        _getMain().entityItemValueListActionNotPerformedWarning([Ent.GROUP, group], [Ent.COPYFROM_GROUP, copySettingsFromGroup], Msg.API_ERROR_SETTINGS)
        return None
      except GAPI.notFound:
        _getMain().entityItemValueListActionNotPerformedWarning([Ent.GROUP, group], [Ent.COPYFROM_GROUP, copySettingsFromGroup], Msg.DOES_NOT_EXIST)
        return None
        ...
        _getMain().entityItemValueListActionNotPerformedWarning([Ent.GROUP, group], [Ent.COPYFROM_GROUP, copySettingsFromGroup], str(e))
        return None
  return gs_body
...
def checkReplyToCustom(group, settings, i=0, count=0):
  if settings.get('replyTo') != 'REPLY_TO_CUSTOM' or settings.get('customReplyTo', ''):
    return True
  _getMain().entityActionNotPerformedWarning([Ent.GROUP, group], Msg.REPLY_TO_CUSTOM_REQUIRES_EMAIL_ADDRESS, i, count)
  return False
...
def doCreateGroup(ciGroupsAPI=False):
  def waitingForCreationToComplete(sleep_time):
    _getMain().writeStderr(Ind.Spaces()+Msg.WAITING_FOR_ITEM_CREATION_TO_COMPLETE_SLEEPING.format(Ent.Singular(Ent.GROUP), sleep_time))
  ...
  groupEmail = _getMain().getEmailAddress(noUid=True)
  ...
  while Cmd.ArgumentsRemaining():
    ...
    if myarg == 'getbeforeupdate':
      ...
      gs_body.update(_getMain().getJSON(GROUP_JSON_SKIP_FIELDS))
    elif myarg == 'accesstype':
      gs_body.update(_getMain().getChoice(GROUP_ACCESS_TYPE_CHOICE_MAP, mapChoice=True))
    elif ciGroupsAPI and myarg in ['alias', 'aliases']:
      body.setdefault('additionalGroupKeys', [])
      for alias in _getMain().convertEntityToList(_getMain().getString(Cmd.OB_CIGROUP_ALIAS_LIST), shlexSplit=True):
      ...
      recentDeleteRetries = _getMain().getInteger(minVal=0, maxVal=20)
    elif myarg == 'recentdeleteretrydelay':
      recentDeleteRetryDelay = _getMain().getInteger(minVal=1, maxVal=60)
    elif myarg == 'verifycreationretries':
      verifyCreationRetries = _getMain().getInteger(minVal=0, maxVal=20)
    elif myarg == 'verifycreationinitialdelay':
      verifyCreationInitialDelay = _getMain().getInteger(minVal=0, maxVal=60)
    elif myarg == 'verifycreationretrydelay':
      verifyCreationRetryDelay = _getMain().getInteger(minVal=1, maxVal=60)
    else:
      getGroupAttrValue(myarg, gs_body)
  if verifyNotInvitable:
    isInvitableUser, _ = _getMain()._getIsInvitableUser(None, groupEmail)
    if isInvitableUser:
      _getMain().entityActionNotPerformedWarning([Ent.GROUP, groupEmail], Msg.EMAIL_ADDRESS_IS_UNMANAGED_ACCOUNT)
      return
  if ciGroupsAPI:
  ...
  while True:
    try:
      if not ciGroupsAPI:
        _getMain().callGAPI(cd.groups(), 'insert',
                 throwReasons=GAPI.GROUP_CREATE_THROW_REASONS,
                 body=body, fields='')
      else:
        _getMain().callGAPI(ci.groups(), 'create',
      ...
      if gs_body and not GroupIsAbuseOrPostmaster(groupEmail):
        ...
        if getBeforeUpdate:
          ...
          settings.update(gs_body)
        _getMain().callGAPI(gs.groups(), 'update',
      ...
      _getMain().entityActionPerformed([entityType, groupEmail])
      break
      ...
      if ciGroupsAPI or duplicateRetries > recentDeleteRetries:
        _getMain().entityActionFailedWarning([entityType, groupEmail], str(e))
        break
...
#      entityActionFailedWarning([entityType, groupEmail], Msg.DOES_NOT_EXIST)
      ...
      _getMain().entityActionFailedWarning([entityType, groupEmail], Msg.INVALID_JSON_SETTING)
      break
  if ciGroupsAPI or not verifyCreationRetries:
    return
  Act.Set(Act.VERIFYITEMEXISTS)
  action = Act.Get()
  _getMain().performAction(Ent.GROUP, groupEmail)
  Ind.Increment()
  ...
  while True:
    try:
      ...
      _getMain().entityActionPerformed([Ent.GROUP, groupEmail])
      break
    except GAPI.groupNotFound:
      retries += 1
      kvList = [Act.PerformedName(action), False, 'Retry', f'{retries}/{verifyCreationRetries}']
      _getMain().printEntityKVList([Ent.GROUP, groupEmail], kvList)
      if retries >= verifyCreationRetries:
        _getMain().entityActionFailedWarning([Ent.GROUP, groupEmail], Msg.RETRIES_EXHAUSTED.format(verifyCreationRetries))
        break
      waitingForCreationToComplete(verifyCreationRetryDelay)
            ...
            GAPI.badRequest, GAPI.permissionDenied, GAPI.systemError, GAPI.serviceLimit, GAPI.serviceNotAvailable, GAPI.authError) as e:
      _getMain().entityActionFailedWarning([Ent.GROUP, groupEmail], str(e))
      break
    except KeyboardInterrupt:
      _getMain().entityActionFailedWarning([Ent.GROUP, groupEmail], Msg.CHECK_INTERRUPTED)
      break
  Ind.Decrement()

# [addonly|removeonly]
def getSyncOperation():
  return _getMain().getChoice(['addonly', 'removeonly'], defaultChoice='addremove')
...
def doUpdateGroups():
  ...
  def _getPreviewActionCSV():
    preview = _getMain().checkArgumentPresent('preview')
    if _getMain().checkArgumentPresent('actioncsv'):
      csvPF = _getMain().CSVPrintFile(GROUP_PREVIEW_TITLES)
    else:
      csvPF = None
  ...
  def _validateSubkeyRoleGetMembers(group, role, origGroup, groupMemberLists, i, count):
    ...
    _getMain().entityActionNotPerformedWarning([entityType, group, Ent.ROLE, role], Msg.INVALID_ROLE.format(','.join(sorted(GROUP_ROLES_MAP))), i, count)
    return (None, None)

  def _getRoleGroupMemberType(defaultRole=Ent.ROLE_MEMBER, allowIgnoreRole=False):
    if not allowIgnoreRole or not _getMain().checkArgumentPresent(['ignorerole']):
      role = _getMain().getChoice(GROUP_ROLES_MAP, defaultChoice=defaultRole, mapChoice=True)
    else:
      role = Ent.ROLE_ALL
    groupMemberType = _getMain().getChoice({'usersonly': Ent.TYPE_USER, 'groupsonly': Ent.TYPE_GROUP}, defaultChoice='ALL', mapChoice=True)
...
# Convert foo@googlemail.com to foo@gmail.com; eliminate periods in name for foo.bar@gmail.com
  ...
  def _previewAction(group, members, role, jcount, action):
    ...
    for member in members:
      j += 1
      _getMain().entityActionPerformed([entityType, group, role, member], j, jcount)
    Ind.Decrement()
    if csvPF:
      for member in members:
  ...
  def _showSuccess(group, member, role, delivery_settings, j, jcount, optMsg=None):
    ...
    if delivery_settings != _getMain().DELIVERY_SETTINGS_UNDEFINED:
      kvList.append(f'{Ent.Singular(Ent.DELIVERY)}: {delivery_settings}')
    if optMsg:
      kvList.append(optMsg)
    _getMain().entityActionPerformedMessage([entityType, group, Ent.MEMBER, member], ', '.join(kvList), j, jcount)
    if csvPF:
  ...
  def _showFailure(group, member, role, errMsg, j, jcount):
    _getMain().entityActionFailedWarning([entityType, group, Ent.MEMBER, member, Ent.ROLE, role], errMsg, j, jcount)
    if csvPF:
  ...
  def _addMember(group, i, count, role, delivery_settings, member, j, jcount):
    ...
    try:
      ...
      _getMain().entityUnknownWarning(entityType, group, i, count)
  ...
  def _handleDuplicateAdd(group, i, count, role, delivery_settings, member, j, jcount):
    try:
      ...
      pass
    _getMain().printEntityKVList([entityType, group, Ent.MEMBER, member], [Msg.MEMBERSHIP_IS_PENDING_WILL_DELETE_ADD_TO_ACCEPT], j, jcount)
    try:
  ...
  def _callbackAddGroupMembers(request_id, _, exception):
    ...
    if exception is None:
      ...
      if reason in GAPI.MEMBERS_THROW_REASONS and reason != GAPI.SERVICE_NOT_AVAILABLE:
        _getMain().entityUnknownWarning(entityType, ri[RI_ENTITY], int(ri[RI_I]), int(ri[RI_COUNT]))
        ...
        errMsg = _getMain().getHTTPError(_ADD_MEMBER_REASON_TO_MESSAGE_MAP, http_status, reason, message)
        ...
        if addBatchParms['adjust']:
          ...
          _getMain().writeStderr(f'{WARNING_PREFIX}{Msg.INTER_BATCH_WAIT_INCREASED.format(addBatchParms["wait"])}\n')
        time.sleep(0.1)
  ...
  def _batchAddGroupMembers(group, i, count, addMembers, role, delivery_settings):
    ...
    _getMain().entityPerformActionNumItems([entityType, group], jcount, role, i, count)
    if jcount == 0:
      return
    if preview:
    ...
    for member in addMembers:
      ...
      dbatch.add(method(**svcparms), request_id=_getMain().batchRequestID(group, i, count, j, jcount, member, role, delivery_settings))
      bcount += 1
  ...
  def _removeMember(group, i, count, role, member, j, jcount):
    try:
      _getMain().callGAPI(cd.members(), 'delete',
               ...
               retryReasons=GAPI.MEMBERS_RETRY_REASONS,
               groupKey=group, memberKey=member)
      _showSuccess(group, member, role, _getMain().DELIVERY_SETTINGS_UNDEFINED, j, jcount)
    except (GAPI.groupNotFound, GAPI.domainNotFound, GAPI.domainCannotUseApis, GAPI.invalid, GAPI.forbidden):
      _getMain().entityUnknownWarning(entityType, group, i, count)
    except (GAPI.memberNotFound, GAPI.invalidMember, GAPI.resourceNotFound, GAPI.conditionNotMet, GAPI.serviceNotAvailable) as e:
      _showFailure(group, member, role, str(e), j, jcount)
    except GAPI.conflict:
      _showSuccess(group, member, role, _getMain().DELIVERY_SETTINGS_UNDEFINED, j, jcount, Msg.ACTION_MAY_BE_DELAYED)
  ...
  def _callbackRemoveGroupMembers(request_id, _, exception):
    ri = request_id.splitlines()
    if exception is None:
      _showSuccess(ri[RI_ENTITY], ri[RI_ITEM], ri[RI_ROLE], _getMain().DELIVERY_SETTINGS_UNDEFINED, int(ri[RI_J]), int(ri[RI_JCOUNT]))
    else:
      http_status, reason, message = _getMain().checkGAPIError(exception)
      if reason in GAPI.MEMBERS_THROW_REASONS and reason != GAPI.SERVICE_NOT_AVAILABLE:
        _getMain().entityUnknownWarning(entityType, ri[RI_ENTITY], int(ri[RI_I]), int(ri[RI_COUNT]))
      elif reason == GAPI.CONFLICT:
        _showSuccess(ri[RI_ENTITY], ri[RI_ITEM], ri[RI_ROLE], _getMain().DELIVERY_SETTINGS_UNDEFINED, int(ri[RI_J]), int(ri[RI_JCOUNT]), Msg.ACTION_MAY_BE_DELAYED)
      elif reason not in GAPI.DEFAULT_RETRY_REASONS+GAPI.MEMBERS_RETRY_REASONS+[GAPI.RESOURCE_NOT_FOUND]:
        errMsg = _getMain().getHTTPError(_REMOVE_MEMBER_REASON_TO_MESSAGE_MAP, http_status, reason, message)
        _showFailure(ri[RI_ENTITY], ri[RI_ITEM], ri[RI_ROLE], errMsg, int(ri[RI_J]), int(ri[RI_JCOUNT]))
      else:
        if remBatchParms['adjust']:
          remBatchParms['adjust'] = False
          remBatchParms['wait'] += 0.25
          _getMain().writeStderr(f'{WARNING_PREFIX}{Msg.INTER_BATCH_WAIT_INCREASED.format(remBatchParms["wait"])}\n')
        time.sleep(0.1)
  ...
  def _batchRemoveGroupMembers(group, i, count, removeMembers, role, qualifier=''):
    ...
    if not qualifier:
      ...
      _getMain().entityPerformActionNumItemsModifier([entityType, group], jcount, role, qualifier, i, count)
    if jcount == 0:
      return
    if preview:
    ...
    for member in removeMembers:
      ...
      dbatch.add(method(**svcparms), request_id=_getMain().batchRequestID(group, i, count, j, jcount, svcparms['memberKey'], role))
      bcount += 1
  ...
  def _callbackUpdateGroupMembers(request_id, _, exception):
    ...
    if exception is None:
      ...
      if reason == GAPI.MEMBER_NOT_FOUND and createIfNotFound:
        ...
        errMsg = _getMain().getHTTPError(_UPDATE_MEMBER_REASON_TO_MESSAGE_MAP, http_status, reason, message)
        _showFailure(ri[RI_ENTITY], ri[RI_ITEM], ri[RI_ROLE], errMsg, int(ri[RI_J]), int(ri[RI_JCOUNT]))
      else:
        if addBatchParms['adjust']:
          addBatchParms['adjust'] = False
  ...
  def _batchUpdateGroupMembers(group, i, count, updateMembers, role, delivery_settings):
    ...
    _getMain().entityPerformActionNumItems([entityType, group], jcount, Ent.MEMBER, i, count)
    if jcount == 0:
      return
    if preview:
    ...
    for member in updateMembers:
      ...
      svcparms = svcargs.copy()
      svcparms['memberKey'] = member
      dbatch.add(method(**svcparms), request_id=_getMain().batchRequestID(group, i, count, j, jcount, svcparms['memberKey'], role, delivery_settings))
      bcount += 1
  ...
  CL_subCommand = _getMain().getChoice(UPDATE_GROUP_SUBCMDS, defaultChoice=None)
  ...
  if not CL_subCommand:
    ...
    while Cmd.ArgumentsRemaining():
      myarg = _getMain().getArgument()
      if myarg == 'email':
        body['email'] = _getMain().getEmailAddress(noUid=True)
        ...
        body['adminCreated'] = _getMain().getBoolean()
    ...
    for group in entityList:
      ...
      if updatePrimaryEmail:
        if updatePrimaryEmail[0].search(group) is not None:
          ...
          if updatePrimaryEmail[2]:
            _getMain().entityActionNotPerformedWarning([Ent.GROUP, group], Msg.UPDATE_PRIMARY_EMAIL_PREVIEW.format(body['email']), i, count)
            continue
        else:
          _getMain().entityActionNotPerformedWarning([Ent.GROUP, group], Msg.PRIMARY_EMAIL_DID_NOT_MATCH_PATTERN.format(updatePrimaryEmail[0].pattern), i, count)
          continue
      if 'email' in body and verifyNotInvitable:
        isInvitableUser, _ = _getMain()._getIsInvitableUser(None, body['email'])
        if isInvitableUser:
          _getMain().entityActionNotPerformedWarning([Ent.GROUP, body['email']], Msg.EMAIL_ADDRESS_IS_UNMANAGED_ACCOUNT)
          continue
      ...
      if gs_body and not GroupIsAbuseOrPostmaster(group):
        try:
          ...
          _getMain().entityActionFailedWarning([entityType, origGroup], Msg.DOES_NOT_EXIST, i, count)
          continue
        except (GAPI.groupNotFound, GAPI.domainNotFound, GAPI.domainCannotUseApis, GAPI.forbidden,
                GAPI.backendError, GAPI.invalid, GAPI.invalidInput, GAPI.badRequest, GAPI.permissionDenied,
                GAPI.systemError, GAPI.serviceLimit, GAPI.serviceNotAvailable, GAPI.authError) as e:
          _getMain().entityActionFailedWarning([entityType, origGroup], str(e), i, count)
          continue
      if body:
        try:
          group = _getMain().callGAPI(cd.groups(), 'update',
      ...
      if gs_body and not GroupIsAbuseOrPostmaster(group):
        try:
          ...
          _getMain().entityActionFailedWarning([entityType, origGroup], Msg.INVALID_JSON_SETTING, i, count)
          continue
      if ci_body:
        ...
        if not name or not groupEmail:
          continue
        try:
          _getMain().callGAPI(ci.groups(), 'patch',
          ...
          continue
      _getMain().entityActionPerformed([entityType, origGroup], i, count)
  elif CL_subCommand in {'create', 'add'}:
    baseRole, groupMemberType = _getRoleGroupMemberType()
    isSuspended, isArchived = _getMain()._getOptionalIsSuspendedIsArchived()
    delivery_settings = _getMain().getDeliverySettings()
    preview, csvPF = _getPreviewActionCSV()
    _, addMembers = _getMain().getEntityToModify(defaultEntityType=Cmd.ENTITY_USERS,
    ...
    for group in entityList:
      ...
      origGroup = group
      ci, _, group = _getMain().checkGroupExists(cd, ci, False, group, i, count)
      if not group:
        continue
      for role in roleList:
        ...
        _batchAddGroupMembers(group, i, count,
                              [_getMain().convertUIDtoEmailAddress(member, cd=cd, emailTypes='any',
    ...
    isSuspended, isArchived = _getMain()._getOptionalIsSuspendedIsArchived()
    preview, csvPF = _getPreviewActionCSV()
    _, removeMembers = _getMain().getEntityToModify(defaultEntityType=Cmd.ENTITY_USERS,
                                         isSuspended=isSuspended, isArchived=isArchived, groupMemberType=groupMemberType)
    ...
    isSuspended, isArchived = _getMain()._getOptionalIsSuspendedIsArchived()
    removeDomainNoStatusMembers = _getMain().checkArgumentPresent('removedomainnostatusmembers')
    delivery_settings = _getMain().getDeliverySettings()
    preview, csvPF = _getPreviewActionCSV()
    additionalMembers = {}
    while _getMain().checkArgumentPresent('additionalmembers'):
      additionalRole = _getMain().getChoice(GROUP_ROLES_MAP, defaultChoice=baseRole, mapChoice=True)
      additionalMembers.setdefault(additionalRole, [])
      additionalMembers[additionalRole].extend(_getMain().getEntityList(Cmd.OB_EMAIL_ADDRESS_ENTITY))
    _, syncMembers = _getMain().getEntityToModify(defaultEntityType=Cmd.ENTITY_USERS,
                                       isSuspended=isSuspended, isArchived=isArchived, groupMemberType=groupMemberType)
    ...
    if groupMemberLists is None:
      ...
      for member in syncMembers:
        syncMembersSets[baseRole].add(_cleanConsumerAddress(_getMain().convertUIDtoEmailAddress(member, cd=cd, emailTypes='any',
    ...
    for group in entityList:
      ...
      for role in roleList:
        ...
        if groupMemberLists is None:
          ...
          if not subkeyRoleField:
            ...
            rolesSet.add(role)
          for member in syncMembers:
            syncMembersSets[role].add(_cleanConsumerAddress(_getMain().convertUIDtoEmailAddress(member, cd=cd, emailTypes='any',
      ...
      _getMain().printGettingAllEntityItemsForWhom(memberRoles, group, entityType=entityType)
      try:
      ...
      for member in result:
        ...
        if groupMemberType in ('ALL', member['type']) and role in rolesSet:
          if not removeDomainNoStatusMembers or memberStatus != 'NONE':
            if _getMain()._checkMemberStatusIsSuspendedIsArchived(memberStatus, isSuspended, isArchived):
    ...
    delivery_settings = _getMain().getDeliverySettings()
    preview, csvPF = _getPreviewActionCSV()
    createIfNotFound = _getMain().checkArgumentPresent('createifnotfound')
    _, updateMembers = _getMain().getEntityToModify(defaultEntityType=Cmd.ENTITY_USERS,
                                         isSuspended=isSuspended, isArchived=isArchived, groupMemberType=groupMemberType)
    ...
    for group in entityList:
      ...
      for role in roleList:
        ...
        _batchUpdateGroupMembers(group, i, count,
                                 [_getMain().convertUIDtoEmailAddress(member, cd=cd, emailTypes='any', checkForCustomerId=True) for member in updateMembers],
                                 role, delivery_settings)
  else: #clear
    rolesSet = set()
    groupMemberType = 'ALL'
    ...
    while Cmd.ArgumentsRemaining():
      ...
      if myarg in GROUP_ROLES_MAP:
        ...
        emailMatchPattern = _getMain().getREPattern(re.IGNORECASE)
...
def verifyGroupPrimaryEmail(cd, group, i, count):
  try:
    ...
    if (result['email'].lower() == group) or (result['id'] == group):
      return True
    _getMain().entityActionNotPerformedWarning([Ent.GROUP, group], Msg.NOT_A_PRIMARY_EMAIL_ADDRESS, i, count)
    return False
  except (GAPI.groupNotFound, GAPI.domainNotFound, GAPI.domainCannotUseApis, GAPI.forbidden,
          GAPI.badRequest, GAPI.backendError, GAPI.systemError):
    pass
  _getMain().entityUnknownWarning(Ent.GROUP, group, i, count)
  return False
...
def getGroupMemberTypes(myarg, typesSet):
  if myarg in {'type', 'types'}:
    for gtype in _getMain().getString(Cmd.OB_GROUP_TYPE_LIST).lower().replace(',', ' ').split():
      if gtype in GROUP_MEMBER_TYPES_MAP:
        typesSet.add(GROUP_MEMBER_TYPES_MAP[gtype])
      else:
        _getMain().invalidChoiceExit(gtype, GROUP_MEMBER_TYPES_MAP, True)
  else:
    return False
  return True
