"""Command-level functions for fetching data from Google Docs, Sheets, and Cloud Storage.

These are composite functions that combine CLI argument parsing, API authentication,
and data retrieval.  They belong in the ``cmd`` layer because they call
:func:`getDriveFileEntity` and :func:`_validateUserGetFileIDs` from
``cmd.drive.core``.

Functions in this module are imported by both ``cmd/`` modules (legal) and
``util/batch.py`` / ``util/entity.py`` (documented transitional violations —
those util modules contain command-handler code that should eventually migrate
to ``cmd/``).
"""
...
from gam.constants import DEFAULT_CSV_READ_MODE, MIMETYPE_GA_SPREADSHEET, NO_ENTITIES_FOUND_RC
...
from gam.util.display import userDriveServiceNotEnabledWarning
from gam.util.errors import entityActionFailedExit, entityDoesNotExistExit
from gam.util.fileio import (
    FILE_ERROR_RC,
    UTF8_SIG,
    fileErrorMessage,
    getGDocSheetDataFailedExit,
    getGDocSheetDataRetryWarning,
    openFile,
    readFile,
    setFilePath,
)
from gam.util.output import stderrWarningMsg, systemErrorExit
from gam.util.api import buildGAPIObject
from gam.util.svcacct import buildGAPIServiceObject
from gam.util.api_call import callGAPI

from gam.util.gdoc import (
...
# Same-layer import (cmd → cmd) — legal
from gam.cmd.drive.core import getDriveFileEntity, _validateUserGetFileIDs
...
# ---------------------------------------------------------------------------
# Google Docs / Drive file data fetching
# ---------------------------------------------------------------------------

def getGDocData(gformat):
  """Fetch content from a Google Doc or Drive file as a TemporaryFile.

  Parses CLI args (user email, file entity), authenticates, resolves the file
  ID, then downloads or exports the content.
  """
...
def getGSheetData():
  """Fetch content from a Google Sheet as CSV in a TemporaryFile."""
  user = getEmailAddress()
  fileIdEntity = getDriveFileEntity(queryShortcutsOK=False)
...
# ---------------------------------------------------------------------------
# Cloud Storage data fetching
# ---------------------------------------------------------------------------

def getStorageFileData(gcsformat, returnData=True):
  """Fetch content from a Cloud Storage object."""
...
# ---------------------------------------------------------------------------
# CSV file reading (local + remote dispatch)
# ---------------------------------------------------------------------------

def openCSVFileReader(filename, fieldnames=None):
  """Open a CSV file for reading.

  Handles local files, Google Docs (``gcsv``/``gdoc``), Google Sheets
  (``gsheet``), and Cloud Storage files (``gcscsv``/``gcsdoc``).
  """
...
# ---------------------------------------------------------------------------
# String-or-file argument functions
# ---------------------------------------------------------------------------

def getStringOrFile(myarg, minLen=0, unescapeCRLF=False):
  """Read a string value from CLI args, a local file, Google Doc, or Cloud Storage."""
...
def getStringWithCRsNLsOrFile():
  """Read a string or file contents, unescaping CR/NL sequences."""
  if checkArgumentPresent(SORF_FILE_ARGUMENTS):
