...
"""GAM YubiKey management commands and hardware interface.

This module contains both the YubiKey hardware abstraction (the ``YubiKey``
class) and the GAM CLI command handler (``doResetYubiKeyPIV``).  All imports
of the optional ``yubikey-manager`` / ``pyscard`` packages are kept at module
level here; callers must use a **deferred import** of this module so that GAM
can start without these packages installed.
"""
...
# --- GAM CLI command handler ---

from gam.var import Cmd  # noqa: E402
from gam.util.args import getArgument, getInteger  # noqa: E402
from gam.util.errors import unknownArgumentExit  # noqa: E402

def doResetYubiKeyPIV():
  """Reset YubiKey PIV application and generate a new signing key."""
  new_data = {}
